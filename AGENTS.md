...
- **`KVARN_HEAD_DIMS`** = `(128, 256, 512)` — the head dims KVarN can quantize (`llama_kvarn_head_slices`: the kernels rotate each head through a Walsh-Hadamard transform in fixed 128-element slices), and **`ModelKV.support_kvarn`**, which applies that rule to a geometry. It answers head dims only; the arch-level exclusions (MLA, DSA, DFlash, `_ARCH_NO_KVARN`) are the caller's. The distinction matters because the two failure modes differ: an unsupported _architecture_ starts and silently stores the fallback type, whereas an unsupported _head dim_ makes llama-server refuse to start (`fail_if_unsupported` defaults to true and has no CLI override) — so the whole 64-dim-head LFM2 family cannot use `-ctk kvarnN` at all.
