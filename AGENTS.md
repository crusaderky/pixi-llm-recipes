...
- **`ARCH_LAZY_TENSORS`** — the tensors an architecture creates with `TENSOR_READ_LAZY`: `per_layer_token_embd.weight` on `gemma4` and `qwen4exp`, `engram_embd.weight` on `deepseek41` (DeepSeek-V4.1-Flash's two ~384M-row Engram n-gram tables, 194.6 GiB at Q8_0 — two fifths of the file) — hash-embedding tables whose byte range llama.cpp registers in `lazy_tensor_ranges` and reads a row at a time out of the mmap as tokens need it. Such a tensor is **weights on disk that never become resident weights**, so it never reaches VRAM — and on Qwen3.8-Flash-Next it is a _quarter of the file_ (26.8 GiB of 103.7 at UD-Q4_K_XL). The flag is arch code, not GGUF metadata; keep the table in step with `git grep -l TENSOR_READ_LAZY src/models/`. Names are matched on the `blk.N.`-stripped form, so a per-layer tensor hits the same entry as a global one.
...
- **`_ARCH_K_ONLY_CACHE`** — architectures forcing K-only storage **without** declaring MLA head dims: `deepseek4` fakes `is_mla()` (`dsv4_make_k_only`) purely to get `has_v = false`, so its `attention.value_length` must be zeroed exactly as for real MLA; `deepseek41`'s `attn_kv` row and compressor output are likewise one fused 512-wide latent per token covering all 64 attention heads (`head_count_kv = 1`, one `sparse_attn` KV entry — confirmed against DeepSeek's own reference implementation), so its declared `attention.value_length` (512) is dead weight too.
- **`_ARCH_NO_KVARN`** — architectures branching to a bespoke cache **before** llama-model.cpp's KVarN test, so they never receive the params (`deepseek32`, `glm-dsa`, `deepseek4`, `deepseek41`). `-ctk kvarnN` there silently stores `KVARN_FALLBACK`, which the report substitutes and labels. deepseek41 has no llama.cpp loader at all yet; its 512/128 head dims would pass the KVarN head-dim test, so without the entry the kvarn rows would be sized as if they were real.
...
- **DeepSeek-V4.1-Flash** (`deepseek41`, no llama.cpp loader yet — sized from the GGUF plus DeepSeek's `inference/model.py`) inverts that layout (CSA2): only the 4 layers carrying `attn_compressor_kv` (2/8/14 at ratio 2, 20 at ratio 1) keep a compressed cache, which **every** ratio-carrying layer reads, so the compressed history is one cache per source layer rather than one per compressed layer. The layers carrying `indexer.k_norm` keep the index-key caches; the bare `indexer.proj`/`attn_q_b` layers reuse them. `_dsv41_compressed_groups()` locates the sources from the tensor table (the metadata does not name them), adds the ratio-2 partial-group compressor ring (`kv_state` + `score_state`, `ratio` rows of f32), and sizes un-padded. Cross-check: the four latents + four index-key caches at fp4 with the checkpoint's scale formats reproduce DeepSeek's published **890 bytes/token** global KV exactly. The Engram tables (layers 1/14) are n-gram _weights_, not cache — they ride in `ARCH_LAZY_TENSORS`.
