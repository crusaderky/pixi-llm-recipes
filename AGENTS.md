...
### Coding-benchmark panel (`benchmarks/`)

Executable implementations of the design docs in `docs/benchmarks/` (read `00`
and `05` first). Each benchmark is a self-contained unit with its own `setup.sh`
(builds an **isolated `uv` venv** + fetches its pinned harness/data — doc 05
§Harness isolation, never the shared pixi env), runner(s), and `README.md`. All
runners import the shared `benchmarks/lib/benchlib` (pins, endpoint, cutoff,
timing, ledger, report, sandbox) by path, talk to an OpenAI-compatible `/v1`
endpoint via `OPENAI_BASE_URL`/`OPENAI_API_KEY`/`BENCH_MODEL` (+ `BENCH_PRECISION`
and `BENCH_T1_FROGGERIC`/`BENCH_T2_REASONING_BUDGET`/`BENCH_T3_KV`/
`BENCH_PARALLEL_SLOTS` for the ledger), append a validated entry to
`docs/benchmarks/results/runs.jsonl`, and write `REPORT-<run_id>.md`.

| Unit                       | Docs   | Arms        | Setup + run                                                                                              |
| -------------------------- | ------ | ----------- | -------------------------------------------------------------------------------------------------------- |
| `benchmarks/ifbench`       | 07     | L1/L2       | `bash setup.sh; .venv/bin/python run_ifbench.py --arm L2` (no sandbox; `--selftest` offline)             |
| `benchmarks/evalplus`      | 08     | L1/L2       | `run_evalplus.py --arm L2 --datasets humaneval,mbpp` (`--smoke-grade` offline; grading in bench-sandbox) |
| `benchmarks/cruxeval`      | 09     | L1/L2       | `run_cruxeval.py --arm L2` (-O literal-eq / -I sandbox; `--selftest` offline)                            |
| `benchmarks/livecodebench` | 06     | L1/L2       | `run_livecodebench.py --arm L2` (cutoff pre-check; grading in bench-sandbox; `--smoke-grade` offline)    |
| `benchmarks/scicode`       | 01, 03 | L1–L5       | `run_canonical.py` (inspect_ai; `--mode dummy` smoke) + `run_pi.py --arm L3/L4/L5`                       |
| `benchmarks/terminalbench` | 02, 04 | L1/L2/L4/L5 | `run.py --arm L2 [--dry]` (Harbor 0.19 + Terminus 2 / pi adapter; Docker)                                |

Untrusted model-generated code is graded through `benchmarks/scripts/bench-sandbox.sh`
(bwrap, no network, `/dev/shm` for grader multiprocessing). Reasoning models on
CPU-offloaded configs decode slowly (~10 tok/s) so full runs are multi-hour —
size them with the doc-00 calibration probe + abort rule; `--no-think` (where
offered) and the offline `--smoke-grade`/`--selftest`/`--dry` modes give fast
plumbing checks.
...
| `benchmarks/scripts/bench-sandbox.sh`                                                     | Untrusted-code exec sandbox for benchmark grading (`docs/benchmarks/05-common-lightweight.md` D2); bwrap, no network, `/dev/shm` + `/tmp`-only writable |
| `benchmarks/scripts/smoke-endpoint.py`                                                    | D1 endpoint smoke: one OpenAI completion against `OPENAI_BASE_URL`/`BENCH_MODEL`                                                                        |
| `benchmarks/lib/benchlib/`                                                                | Shared benchmark library imported by every runner (pins, endpoint, cutoff, timing, ledger, report, sandbox)                                             |
| `benchmarks/<unit>/` (ifbench, evalplus, cruxeval, livecodebench, scicode, terminalbench) | Per-benchmark `setup.sh` + runner(s) + README implementing design docs 01–09 (see §Coding-benchmark panel)                                              |
