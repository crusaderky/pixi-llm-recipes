...
set "TARGET=windows-x86_64.exe"
set "URL=https://github.com/ogulcancelik/herdr/releases/download/%VERSION_PREVIEW%/herdr-%TARGET%"

echo Downloading %URL%...
curl -fsSL --retry 3 --connect-timeout 10 --max-time 120 "%URL%" -o herdr.exe || exit /b 1

echo Verifying sha256...
for /f "tokens=1" %%i in ('certutil -hashfile herdr.exe SHA256 ^| findstr /v "hash"') do set "ACTUAL=%%i"
